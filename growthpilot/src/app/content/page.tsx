    ...
    <div className="mx-auto max-w-6xl pb-10">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.78fr_1.22fr]">
        ...
        <section className="border border-slate-700 bg-slate-900 p-5">
          ...
          {draft ? <><textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows={17} className="mt-5 w-full resize-y border border-slate-700 bg-slate-950/50 p-4 font-mono text-sm leading-6 text-slate-200 outline-none focus:border-cyan-400" /><div className="mt-4 flex flex-col gap-3 border-t border-slate-800 pt-4 sm:flex-row sm:flex-wrap"><input type="date" value={scheduleDate} onChange={(event) => setScheduleDate(event.target.value)} className="w-full border border-slate-700 bg-slate-950/50 px-3 py-2 text-sm text-white outline-none focus:border-cyan-400 sm:w-auto" /><button type="button" disabled={!scheduleDate || isScheduling} onClick={() => void scheduleDraft()} className="inline-flex w-full items-center justify-center gap-2 border border-cyan-400 px-4 py-2 text-sm font-bold text-cyan-200 hover:bg-cyan-400 hover:text-slate-950 disabled:opacity-50 sm:w-auto"><CalendarDays size={16} />{isScheduling ? "Scheduling..." : "Add to calendar"}</button><button type="button" className="inline-flex w-full items-center justify-center gap-2 bg-cyan-400 px-4 py-2 text-sm font-bold text-slate-950 hover:bg-cyan-300 sm:w-auto"><Send size={16} />Send to review</button></div></> : <div className="mt-5 flex min-h-64 items-center justify-center border border-dashed border-slate-700 bg-slate-950/30 p-6 text-center text-sm leading-6 text-slate-500 sm:min-h-80 sm:p-8">Choose a format and topic to generate a draft. Your content will be based on the business opportunities and active channels in this workspace.</div>}
        </section>
      </div>
